...
#V1.9 added new function of filter genes 2022/02/06
#v1.8 update description more precisely by Sergey 2021/12/14
#v1.7 added new function of couting SNPs numbers 2021/11/04
...
#v1.2 added new function of outputing relaxed-phylip format 2021/09/18
...
#Help
parser = argparse.ArgumentParser(description='This script can be applied to extract SNPs from a VCF file by using a single-copy genes ID list and a GFF file into individual fasta file and phy file.')
...
parser.add_argument("-snp",metavar = 'snp_number',type = int ,required= False,help="Does not output genes with the number of SNPs less than was set")
parser.add_argument("-E",metavar = 'Exclude_Gene',type = float ,required= False,help="Exclude genes with >= value -E missing samples")
parser.add_argument("-e",metavar = 'Exclude_Sample',type = float ,required= False,help="Exclude genes with >= value -E missing data")
...
empty_gene.add_argument('-discard',action='store_true',help='Discard the gene if codon1-2 or codon3 is empty')
...
def delEmpty(matchSNP):
	...
	return matchSNP
def exclude(matchSNP):
	base_missing=0
	sample_missing=0
	matchSNP_orig=copy.deepcopy(matchSNP)
	filter_snp_log=open("filter_snp.log","w+")
	filter_gene_log=open("filter_gene.log","w+")
	for e_chr in matchSNP_orig:
		for e_gene in list(matchSNP_orig[e_chr].keys()):
			for e_snp in matchSNP_orig[e_chr][e_gene]:
				for e_base in matchSNP_orig[e_chr][e_gene][e_snp]:
					if e_base[0] == 'N':
						base_missing+=1
				#filter of base_missing
				if args.e is not None:
					snp_len=float(len(matchSNP_orig[e_chr][e_gene][e_snp]))
					#print(snp_len)
					base_missing_rate=float(base_missing)/snp_len
					#print(base_missing_rate)
					if base_missing_rate >= args.e:
						del matchSNP[e_chr][e_gene][e_snp]
						sample_missing+=1
						filter_snp_log.write(e_gene+' '+ e_snp+' has been filtered! Missing_rate= '+str(base_missing_rate)+'\n')
					base_missing=0
			#filter of sample_missing
			if args.E is not None:
				gene_len=float(len(matchSNP_orig[e_chr][e_gene]))
				#print(gene_len)
				gene_missing_rate=float(sample_missing)/gene_len
				#print(gene_missing_rate)
				if gene_missing_rate >= args.E:
					del matchSNP[e_chr][e_gene]
					filter_gene_log.write(e_gene+' has been filtered! Missing_rate= '+str(gene_missing_rate)+'\n')
				sample_missing=0
	filter_snp_log.close()
	filter_gene_log.close()
	return matchSNP
#output function
...
def main():
	...
	if args.E is not None and args.e is not None:
		matchSNP = exclude(matchSNP)
	# output
	outputSNP(matchSNP)
	...
	#output SNPs numbers
	print("The SNPs number is " + str(SNPs_count))
